...
```js
# 1 - create a package.json file (do not forget to delete `type : commonjs`)
npm init -y
# 2 - install the packages we need
npm i -D typescript webpack webpack-cli ts-loader webpack-dev-server
# 3 - create empty config files
...
- Make some deliberate errors in **utils.js** - such as setting the value of `defaultName` to a number - what does webpack do?
- Also note that "source mapping" is turned on - meaning you can see the contents of your original **.ts** files in the Chrome debugger
