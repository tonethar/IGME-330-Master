...
1) To set up the audio graph, play the audio, and analyse the audio, we first need to create a new ES6 module to hold all of the audio-related code. Let's name this module **audio.js**:
  - here's some pseudo-code you can copy/paste to get you started:
...
3) There's a lot of well commented code here:
  - the purpose of this code is to create our "4 node" audio graph (Source (*i.e. MP3*) --> Analyser --> Gain --> Destination  (*i.e. Speakers*)
...
4) In order to use call these audio functions in out main program, you'll need to import them into **main.js**:
...
6) Test the audio module
  - on the right-side of the debugger, you can see the `audio` module and all of its "public" functions, let's test one of them with the program still paused and "broken" :
    - switch to the console
    - type `audio.playCurrentSound()` - hit enter - then hit the debugger's "continue" button - the "adventure" sound should play
    
7) To hook up the play button, add the following code to `setupUI()`:
