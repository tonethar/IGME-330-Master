...
- Here we begin create the Audio Visulizer HW that will become the foundation of your Project 2
...
3) There's a lot of well commented code here:
  ...
  - note that many of the variables and functions are public (have been exported), this is so that **main.js** can pause and play the sound, load a new sound, change the volume, and so on 
...
4) In order to use call these audio functions in our main program, you'll need to import them into **main.js**:
...
7) A button would be a handy way to play and pause the sound. To hook up the play button, add the following code to `setupUI()`:
...
- anyway, reload the page, successively clicking the play button should toggle between "play and "pause" states. 
...
8) Now that we have the audio playing working, be sure to go back and look over [III. Audio Graphs & Sampling](#audio-graphs-and-sampling) as well as the code in **audio.js** and look over how the audio graph was constructed

9) Let's get the volume slider working - here's the code for that - be sure to type in the comments too.
...
- recall that the `oninput` event of the slider is called whenever the user *moves* the slider - they don't have to release the knob (FYI - the event that gets called when the user *releases* the slider knob is `onchange`)
...
- So we have playing/pausing working, and we have an analyser node hooker up to the audio routing graph

### V-A. What does the audio frequency data look like?
