...
1. We have already seen how to enable CORS by adding `header()` calls to our PHP code. Another way is to add the following to the .htaccess file in the same folder as our PHP "Joke Server":
...
- Using our random joke app as a start file:
    ...
    - Hints:
      ...
      - This web service does not have the CORS header turned on, but the app should still work when run off of the desktop
      - To fix this issue, use a proxy server like we did when we covered [PHP-driven JSON Web Service](./demo-php-web-service-json.md) last week - here it the URL to the service again: https://cors-anywhere.herokuapp.com/
